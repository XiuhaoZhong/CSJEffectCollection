...
bool GraphicCamera::init(GLFWwindow *window) {
	string verPath = Utils::getResourcePath("shaders\\camera\\vertexShader.glsl");
	string fragPath = Utils::getResourcePath("shaders\\camera\\fragmentShader.glsl");
...
void GraphicCamera::display(GLFWwindow *window, double currentTime) {
	...
	view = glm::lookAt(glm::vec3(0.0, camZ, camX), glm::vec3(0.0f, 0.0f, 0.0f), glm::vec3(1.0f, 0.0f, 0.0f));
	...
	for (GLuint i = 0; i < 10; i++) {
		glm::mat4 model(1.0f);
		model = glm::translate(model, g_cubePos[i]);
